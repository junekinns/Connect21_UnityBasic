...
{
    // 변수.
    public float attackDistance = 5f;           // 공격 가능 거리(단위 미터).
    public float traceDistance = 10f;           // 추격 시작 거리(단위 미터).

    public float patrolWaitTime = 3f;           // 정찰할 때까지 대기하는 시간(초).
    public float moveRotateDamping = 7f;        // 이동 시 회전 지연 값.

    public float attackRotateDamping = 10f;     // 공격 시 회전 지연 값.
    public float fireRate = 0.2f;               // 발사 간격(초).

    public int maxBullet = 3;                   // 최대 탄약 수.

    public float maxHP = 100f;                  // 체력

    public readonly float patrolSpeed = 1.5f;   // 정찰 시 이동 속도.
    public readonly float traceSpeed = 3.5f;    // 추격 시 이동 속도.
